...
The site at `https://chienthan2vn.github.io/universal-skills/` serves a browsable skill catalog from `docs/index.html`. It is generated by scanning frontmatter from all platforms:

```bash
npm run docs:build
```

The output `docs/index.html` shows all 28+ skills, filterable by platform, with install commands and git commit hash. After building, commit and push the `docs/` folder to publish.
